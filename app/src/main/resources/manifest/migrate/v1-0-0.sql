...
CREATE TABLE `project` (
                           ...
                           `metadata` json DEFAULT NULL COMMENT '扩展信息(JSON字符串)',
...
CREATE TABLE `base_task` (
                             ...
                             `description` varchar(500) DEFAULT '' COMMENT '任务说明（简介）',
                             `meta_schema` json DEFAULT NULL COMMENT '元数据字段定义(JSON数组)',
                             `status` tinyint NOT NULL DEFAULT 1 COMMENT '状态 1-启用 0-禁用',
                             `creator_id` bigint NOT NULL COMMENT '创建人ID',
                             `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
                             `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `item_task_node` (
                                  ...
                                  `meta_schema` json DEFAULT NULL COMMENT '节点元数据字段定义快照(JSON数组)',
                                  `sort` int NOT NULL DEFAULT 0 COMMENT '步骤顺序',
                                  `parallel_sort` int DEFAULT NULL COMMENT '并行组号排序',
                                  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
                                  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `task_submission` (
                                   ...
                                   `metadata` json DEFAULT NULL COMMENT '提交元数据(JSON对象，按节点 meta_schema 组织)',
