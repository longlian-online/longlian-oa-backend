...
Docker 一键开发环境（`task dev`）挂载上面的本地模板，用环境变量把 MySQL/Redis 指到 compose 服务名，不必再配 `.env`。
...
YAML 挂到容器 `config/application.yml`。Atlas 仍要 `ATLAS_DB_URL` / `ATLAS_DEV_DB_URL`，写在 `.env` 里，不要把密钥打进镜像或提交到 git。
