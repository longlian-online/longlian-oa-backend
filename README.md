...
Docker 开发环境挂载本模板，compose 用环境变量覆盖 MySQL/Redis 主机名。生产复制 `devops/application-prod.yml.example` 与 `devops/env.example` 后 `task prod`。测试使用已提交的 `app/src/test/resources/application-test.yml`。
